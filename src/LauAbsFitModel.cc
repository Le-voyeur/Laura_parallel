...
void LauAbsFitModel::setupResultsOutputs( const TString& histFileName, const TString& tableFileName )
{
	...
	std::cout << "INFO in LauAbsFitModel::setupResultsOutputs : Creating fit ntuple." << std::endl;
	//if (fitNtuple_ != 0) {delete fitNtuple_; fitNtuple_ = 0;}
	std::cout<<__LINE__ << "INFO in LauAbsFitModel::setupResultsOutputs : Creating fit ntuple." << std::endl;
...
void LauAbsFitModel::fitExpt()
{
	...
	LauAbsFitter<LauMinuit>::FitStatus fitResult = LauFitter::fitter()->minimise();
...
void LauAbsFitModel::finaliseExperiment( const LauAbsFitter<LauMinuit>::FitStatus& fitStat, const TObjArray* parsFromMaster, const TMatrixD* covMat, TObjArray& parsToMaster )
{
